...
function ZarządzajKategoriami()
{
    ...
    $query = "SELECT id, nazwa FROM sklep ORDER BY nazwa ASC";
    $result = mysqli_query($conn, $query);
...
function DodajKategorie()
{
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        $query = "INSERT INTO sklep (nazwa, matka) VALUES ('$category_name', $parent_id) LIMIT 1";
        ...
        $result = mysqli_query($conn, "SELECT id, nazwa FROM sklep WHERE matka = 0");
        while ($row = mysqli_fetch_assoc($result)) {
...
function DodajProdukt() {
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        $query = "INSERT INTO produkty (tytul, opis, cena_netto, podatek_vat, ilosc_dostepnych_sztuk_w_magazynie, data_wygaśnięcia, gabaryt_produktu, link_zdjecie, kategoria)
                  VALUES ('$tytul', '$opis', $cena_netto, $vat, $ilosc, '$data_wygasniecia', '$gabaryt', '$link_zdjecie', '$kategoria')";
        if (mysqli_query($conn, $query)) {
...
function EdytujProdukt($id) {
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        $query = "UPDATE produkty SET tytul = '$tytul', opis = '$opis', cena_netto = $cena_netto, podatek_vat = $vat,
                  ...
                  link_zdjecie = '$link_zdjecie', kategoria = '$kategoria' WHERE id = $id LIMIT 1";
        if (mysqli_query($conn, $query)) {
        ...
        if ($row = mysqli_fetch_assoc($result)) {
            ...
            $gabaryt = htmlspecialchars($row['gabaryt']);
